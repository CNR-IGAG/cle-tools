				...
				try:
					...
					for x, y in dict_comuni.iteritems():
						if x == cod_istat:
							...
							os.rename(dir_out + os.sep + "progetto_CLE", path_comune)
					...
					project.read(QFileInfo(path_comune + os.sep + "progetto_CLE.qgs"))
					...
					loc_table = QgsMapLayerRegistry.instance().mapLayersByName("vw_localita")[0]
					loc_table.setSubsetString("cod_com='" + codice_com + "'")
					...
					for composer_view in composers:
						...
						map_item_6 = composition.getComposerItemById('prov_cod')
						map_item_6.setText("Codice Provincia: " + codice_prov)
						map_item_7 = composition.getComposerItemById('com_cod')
						map_item_7.setText("Codice Comune: " + codice_com)
...
##					if os.path.exists(dir_out + os.sep + "progetto_CLE"):
##						shutil.rmtree(dir_out + os.sep + "progetto_CLE")
			else:
				QMessageBox.warning(iface.mainWindow(), u'WARNING!', u"The selected directory does not exist!")

	def disableButton(self):
		check_campi = [self.professionista.text(), self.propretario.text(), self.ufficio.text(), self.scala_nom.text(), self.email_prof.text(), self.email_prop.text(), self.dir_output.text(), str(self.comune.currentText())]
		check_value = []

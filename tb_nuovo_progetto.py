...
class nuovo_progetto(QDialog, FORM_CLASS):
        ...
        if result:
            ...
            if os.path.isdir(dir_out):
                try:
                    ...
                    for x, y in dict_comuni.items():
                        if x == cod_istat:
                            ...
                            os.rename(dir_out + os.sep +
                                      "progetto_CLE", path_comune)

                    project = QgsProject.instance()
                    project_path = os.path.join(
                        path_comune, "progetto_CLE.qgs")
                    ...
                    loc_table = QgsProject.instance(
                    ).mapLayersByName("vw_localita")[0]
                    loc_table.setSubsetString(
                        "cod_com='" + codice_com + "' AND cod_prov='" + codice_prov + "'")
                    ...
                    for layout in layouts:
                        ...
                        map_item_6 = layout.itemById(
                            'prov_cod')
                        map_item_6.setText("Codice Provincia: " + codice_prov)
                        map_item_7 = layout.itemById('com_cod')
    ...
    def indice_tab_execute(self, data_meta, regione, codice_regio, provincia, codice_prov, nome, codice_com, professionista, ufficio, propretario):
        ...
        conn.execute(sql)
        conn.execute("""INSERT INTO 'indice'(data_in,regione,cod_reg,provincia,cod_prov,comune,cod_com,soggetto,ufficio,responsabile,ID_CLE) VALUES ('""" + data_meta + """', '""" + regione + """', '""" + codice_regio + """',

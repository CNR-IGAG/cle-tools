...
def freq(dialog, layer, feature):
	...
	if id_indpu.currentText() != '':
		id_indpu.currentIndexChanged.connect(lambda: disableButton_freq(id_indpu, buttonBox))
		disableButton_freq(id_indpu, buttonBox)
	else:
		id_indpu.currentIndexChanged.connect(lambda: disableButton_freq(id_indpu, buttonBox))
...
def disableButton_freq(id_indpu,buttonBox):
